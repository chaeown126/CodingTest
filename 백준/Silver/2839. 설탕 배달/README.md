...
* * *
수학적 연산이 필요한 문제였음. 단순히 5로 먼저 나누면서 남은 거에 3을 나누는 방식이 아니라 **반복문**을 통해 먼저 n%5 != 0 이 아닐 경우 n -= 3을 하고, 이에 따라 3킬로그램의 개수가 1개 증가하므로 count++을 해줌.
이 과정을 n > 0 인 동안 반복하고 결과가 n != 0 이면 n이 5나 3으로 정확히 나눠지지 않는 것이므로 -1을 출력함.
