...
---
배추의 구역이 모두 이어져 있는 것이 아니었기 때문에 모든 배열을 탐색해야 했다.   
사실상 1을 찾으면 count++ 를 하는 거나 다름없었다. (거기에 어느 구역까지 이어져 있는지 dfs로 탐색)   
   
처음엔 0들로 막힌 곳을 어떻게 뚫고 1로 가야하지..? 했는데   
그냥 단순하게 for문 돌리면 됐던 것이었다~~🤣
