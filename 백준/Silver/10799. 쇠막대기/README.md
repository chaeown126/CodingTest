...
---
생각보다 재밌었던 스택 문제! 🤡   
처음엔 구조를 잘못 잡았는데 금방 다른 방향을 생각해내서 비교적 쉽게(?) 풀었던 것 같다.   
처음에 `String` 자료형으로 풀어서 메모리랑 시간뿐만 아니라 채점 하는 데에도 시간이 너무 오래 걸려서 `char`로 바꿔주니 확실히 해소되었다.    
역시 스택은 웬만하면 `char`로 푸는 게 맞는 것 같다.
